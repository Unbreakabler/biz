  ...
  afterInstall(options: any) {
    ...
    if (!this.pathToModule) {
      const warningMessage = oneLine`
        Feature is generated but not used
      `;
      this._writeStatusToUI(chalk.yellow, 'WARNING', warningMessage);
    } else {
      let className = stringUtils.classify(`${options.entity.name}Feature`);
      let fileName = stringUtils.dasherize(`${options.entity.name}.feature`);
